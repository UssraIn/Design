    ...
    internal class Class1
    {
        ...
        static void Main(string[] args)
        {
            ...
            document.Entities.Add(new Line(origin, point1)); //pour relie entr eux
                                                             //   document.Entities.Add(new Line(origin, point7));
          ...
          //  var points = new List<Vector2>();
                    ...
                    using (SQLiteCommand command = new SQLiteCommand("SELECT * FROM Dp", connection))
                    {
                        ...
                        using (SQLiteDataReader reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                //  document.Entities.Add(new Line(P1,P2 ));

                                //  points.Append(x,y);
                                //  points < -data.frame(Value = c(1, 2, 3, 4, 5))
                                //  dbWriteTable(db, Dp, points, Append = true, Row.names = False)

                                //y  Point point = new Point(new Vector2(x, y));
                                // foreach(Vector2 point in point)
                                // document.Entities.Append(new Point (point));
                                ...
                                // polyline.Vertexes.Add(new Polyline2DVertex(point));
            ...
            document.Entities.Add(new Line(origin, point1)); //pour relie
            document.Entities.Add(new Line(origin, point2));

            //connect last point with first
            //  document.Entities.Add(new Line(points[points.Length - 1], points[0]));

            //Add text (piquet point)
            for (int i = 0; i < points.Length - 1; i++)
            {
                double x = points[i].X;
                double y = points[i].Y;
            ...
            for (int i = 0; i < points.Length; i++)
            {
                double x = points[i].X;
                double y = points[i].Y;
                var text = new Text($"{i + 1 + 10}", new Vector2(points[i].X, points[i].Y - (y + 18)), 3);
                document.Entities.Add(text);
            ...
            for (int i = 0; i < points.Length - 1; i++)
            {
                double x = points[i].X;
                double y = points[i].Y;
                var text = new Text($"{points[i].X + i + 1 + 10}", new Vector2(points[i].X, points[i].Y - (y + 28)), 3);
                document.Entities.Add(text);
            }
            // 4eme ligen
            for (int i = 0; i < points.Length; i++)
            {
                double x = points[i].X;
                double y = points[i].Y;
                var text = new Text($"{123 + i}", new Vector2(points[i].X, points[i].Y - (y + 38)), 3);
                document.Entities.Add(text);
            }
